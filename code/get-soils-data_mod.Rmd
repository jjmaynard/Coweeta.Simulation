...
author: "D.E. Beaudette (modified by Jonathan Maynard)"
date: "`r Sys.Date()`"
output:
  html_document:
    mathjax: null
...
x <- read_sf(here('data/vect/coweeta_boundary_buff.shp'))

# basins
b <- read_sf(here('data/vect/Coweeta_Hydrologic_Laboratory.shp'))

# points
p <- read_sf(here('data/vect/Soil_Moisture_Sites_CW.shp'))
...
```{r fig.width=10, fig.height=8}
## save
writeRaster(soiltype, file = here('data/grids/soiltype-soil-series.tif'), options = 'COMPRESS=LZW', overwrite = TRUE, datatype = 'INT1U')
```
...
levelplot(
  s[[1:3]], 
  main = 'Sand, Silt, Clay (RV) 0-100cm\nDominant Component (Numeric)',
  margin = FALSE, 
  scales = list(draw = FALSE), 
  col.regions = viridis
)
...
texture_0100[] <- ssc_to_texcl(sand = s$sandtotal_r[], clay = s$claytotal_r[])

# extract RAT
rat <- levels(texture_0100)[[1]]
...
# re-pack
levels(texture_0100) <- rat

# check: ok
cols <- levels(texture_0100)[[1]]$hex
levelplot(
  texture_0100, 
  att = 'names', col.regions = cols,
  main = 'Soil Texture (fine earth fraction) 0-100cm\nDominant Component (Numeric)',
  margin = FALSE, 
  scales = list(draw = FALSE),
  panel = function(...) {
    panel.levelplot(...)
...
```{r fig.width=10, fig.height=8}
## save
writeRaster(texture_0100, file = here('data/grids/soiltype-texture_0100.tif'), options = 'COMPRESS=LZW', overwrite = TRUE, datatype = 'INT1U')
```
...
s <- get_SDA_property(property = 'Available Water Capacity - Rep Value', method = 'Weighted Average', mukeys = keys, top_depth = 0, bottom_depth = 100)
...
(aws <- deratify(mu, att = 'aws0100wta'))

levelplot(
  aws, 
  main = 'Available Water Storage 0-100cm (cm)\ngSSURGO WCS + SDA',
  margin = FALSE, 
  scales = list(draw = FALSE), 
  col.regions = viridis,
  panel = function(...) {
...
hist(aws, breaks = 20, las = 1, main = 'Available Water Storage 0-100cm (cm)\ngSSURGO WCS + SDA', cex.axis = 0.66)
```
